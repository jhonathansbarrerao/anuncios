...
eventos = [
    {
        "id": 1,
        "titulo": "Confencia de IA",
        "slug": "conferencia-IA",
        "descripcion": "Charla introductoria sobre la IA.",
        "fecha": "2025-09-15",
        "hora": "14:00",
        ...
        "cupo_maximo": 3,
        "asistentes": [
            {"nombre": "Nicolas Galvez", "correo": "Nico123@gmail.com"}
        ],
        "destacado": True,
    }
]
...
def inicio():
    ...
    proximos.sort(key=lambda e: (e.get("destacado") is False, e["fecha"], e["hora"]))
...
def detalle_evento(slug):
    ...
    return render_template("events/detalle.html", evento=evento, restantes=restantes)
...
def crear_evento():
    ...
    if form.validate_on_submit():
        ...
        if buscar_evento_por_slug(slug):
            flash("Ya existe un evento con ese titulo. Cambia el título.", "error")
            return render_template("admin/form_evento.html", form=form)
...
def registrar_evento(slug):
    ...
    if form.validate_on_submit():
        nuevo = {"nombre": form.nombre.data.strip(), "correo": form.correo.data.strip().lower()}
    ...
    return render_template("events/form_registro.html", evento=evento, form=form)
...
def eventos_por_categoria(categoria):
    ...
    filtrados.sort(key=lambda e: (e.get("destacado") is False, e["fecha"], e["hora"]))
    return render_template("events/por_categoria.html", categoria=categoria, eventos=filtrados, categorias=categorias)
